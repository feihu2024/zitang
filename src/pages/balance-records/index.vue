  ...
  <view class="page ledger-page">
    ...
    <view class="summary-card">
      ...
      <view class="summary-head">
        <text class="summary-label">可用余额（元）</text>
        <view class="frozen-chip">
          <view class="frozen-chip-dot"></view>
          <text class="frozen-chip-text">冻结中 ¥{{ formatMoney(summary.frozenBalance) }}</text>
        </view>
      </view>
    ...
    <view class="ledger-list">
      ...
      <view v-if="!loading && !items.length" class="empty-box">
        ...
        <text class="empty-text">暂无资金明细</text>
        <text class="empty-sub">卖出商品获得佣金后，可在这里查看</text>
      </view>
...
export default {
  data() {
    return {
      summary: { balance: 0, frozenBalance: 0 },
      items: [],
      page: 1,
      size: 20,
      total: 0,
  ...
  methods: {
    async load(append) {
      this.loading = true
      try {
        const resp = await get('/api/wxapp/wallet/balance-records', {
          page: this.page,
          size: this.size
        })
